// dsh-tools-panel —— Client 半区(零构建动态插件)。纯 JavaScript,禁止 import/TS/JSX。
return {
  apply(ctx) {
    ...
    function ToolPanel() {
      ...
      React.useEffect(function () {
        ...
        items.forEach(function (item) {
          if (item.icon) return
          host.call('tools.icon', { path: item.path }).then(function (res) {
            if (res && res.dataUrl) {
      ...
      const doRun = async function (item) {
        set({ open: false })
        try {
          await host.call('tools.run', { path: item.path })
        } catch (err) {}
      }
      const doAdd = async function () {
        ...
        let res = null
        try { res = await host.call('tools.pick') } catch (err) { res = { ok: false, error: String((err && err.message) || err) } }
        set({ busy: false })
        if (res && res.path) {
          ...
          let icon = null
          try { const ic = await host.call('tools.icon', { path: res.path }); if (ic && ic.dataUrl) icon = ic.dataUrl } catch (e) {}
    ...
    styles.insert(`
      .dsh-tool-btn { display:inline-flex; align-items:center; gap:6px; padding:6px 12px; font-size:13px; border-radius:999px; border:1px solid var(--dsw-alias-border-l2); background:transparent; color:var(--dsw-alias-label-secondary); cursor:pointer; }
